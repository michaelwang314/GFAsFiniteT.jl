...
function run!()
    ...
    m_params = [("morse particle", "morse particle", Dict("D0" => D0, "α" => α, "r0" => 0.0, "r_cut" => m_r_cut))]
    m = Morse(m_params, m_particle_pair, m_cell_list, box, false)

    k = 1.0
    r0 = 1.0
    hb_particle_pair = [Particle(box ./ 2, "harmonic particle"), Particle(box ./ 2, "harmonic particle")]
    hb_imatrix = create_interaction_matrix([("harmonic particle", "harmonic_particle")])
    bond_list = bind_closest(hb_particle_pair, 0.01, hb_imatrix)
    ...
    export_trajectories!(trajectories, "TEST_OUTPUT/trajectories.txt")
end
run!()